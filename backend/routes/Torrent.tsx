...
const torrentGroup = (main: Elysia<any>, db: iDatabaseHandler) => {
  main.group("/api/torrent", (app) =>
    app
      .get("/", async (handle): Promise<iReducedTorrent[]> => {
      ...
      .post("/", async (handler) => {
        ...
        if (!Array.isArray(payload.files)) payload.files = [payload.files];

        const clientId = new mongoose.Types.ObjectId(payload.clientId);
        if (
          !(await db.getClients()).some((client) =>
            client._id?.equals(clientId)
          )
        )
        ...
        torrents.forEach((torrent) => {
          ...
          torrent.downloaded = Math.floor(
            (parseInt(payload.progress) / 100) * torrent.size
          );
